...
Deface::Override.new(:virtual_path => 'spree/products/_multi_item_cart_form_element',
                     ...
                     :text => '
                 <h4 class="product-section-title">Sizes</h4>
                 <% id = @product.variants_and_option_values_with_stock(current_currency).collect{ |v| ["#{variant_options(v).downcase}", v.id] }.to_h[Spree::Babe.find(session[:babe_id]).size_value_for_size_option_type_name(@product.product_size_type.name).downcase] if session.key?(:babe_id)%>
