...
describe Spree::Product, :type => :model do
  ...
  context "product has sizes" do
    ...
    let(:bra_option_type) do
      build_option_type_with_values("Bra Sizes", "Size", %w(34A 34B))
    end
    ...
    it "should return the variant id of the matching size" do
      expected_id = product1b.variants_and_option_values_with_stock(nil).collect{ |v| ["#{v.options_text_by_name.downcase}", v.id] }.to_h['medium']
  ...
  context "get sku appending string from variant combination"  do
    ...
    let!(:option_type2) do
      build_option_type_with_values("Colors", "Color", %w(Red))
    end
